...
5. **テストはローカルではなく CI で回す（2026-09-02 方針）。**
   並行セッションが多いとローカルの実行が数倍に劣化し、CI の方が速く安定する
   （実測: ローカル8分43秒 / CI 4分50秒）。手元に残すのは lint・analyze まで。
   **合否の判断は CI のテストに置く。**
   **ただしブランチへの push では CI は走らない（2026-09-08 変更）。**
   Actions の無料枠（月2000分）を1週間で使い切ったため、テストの発火を
   **master への push と Pull Request** に絞った。ブランチで確認したいときは
   **PR を作る**か `workflow_dispatch` で回す。反復は対象を絞って短く
   （soccer-ci.yml が先例）。詳細と切り分け手順は `docs/session-faq.md`。
6. **疑問はユーザーに聞く前に、まず `docs/session-faq.md` を読む。**
   載っていなければ**自分で判断する**。**調整役は置いていない**（2026-09-07 廃止）。
   判断できないもの・ユーザーにしか決められないもの（公開・課金・提出・方針）は
   ユーザーに直接聞く。マージも各セッションが自分で行う（規約4）。
