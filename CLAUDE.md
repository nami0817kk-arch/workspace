...
5. **テストはローカルではなく CI で回す（2026-09-02 方針）。**
   ...
   **合否の判断は CI のテストに置く。** ブランチへの push でもテストは走る。
   反復は対象を絞って短く（soccer-ci.yml の workflow_dispatch が先例）。
   同じブランチで新しい実行が始まると古い実行は打ち切られる（`concurrency`）。
   詳細と切り分け手順は `docs/session-faq.md`。
