...
def test_fetch_step_downloads_and_records_paths(tmp_path, monkeypatch):
    ...
    from imagegen import assets as assets_module
...
# --- say ---------------------------------------------------------------
def test_say_step_writes_audio_and_credits(tmp_path):
    recipe = {
        "name": "読み上げ",
        "steps": [
            {
    ...
    assert item["provider"] == "beep"
    assert item["seconds"] > 0  # 動画に並べるので尺を返す
    assert result.files() == [item["path"]]


def test_say_step_needs_text(tmp_path):
    with pytest.raises(ConfigError, match="text が必要"):
        recipes.run({"steps": [{"say": {"provider": "beep", "out": str(tmp_path)}}]})


def test_say_step_can_use_a_previous_step(tmp_path, monkeypatch):
    """集めた見出しをそのまま読み上げる（feed → say）。"""
    from imagegen.core.types import FeedItem
    ...
    recipe = {
        "steps": [
            ...
            {"say": {"text": "{{ news.0.title }}", "provider": "beep", "out": str(tmp_path)}},
        ]
    }
    result = recipes.run(recipe)
    assert result.steps[1].items[0]["text"] == "移籍が決まりました"
