...
from imagegen import mcp_server
from imagegen.core.types import PublishResult


@pytest.fixture
def output_file():
    """MCP から送れるのはプロジェクト配下のファイルだけなので、出力先に置く。"""
    from imagegen.config import output_dir
...
def test_initialize_returns_server_info():
    ...
    assert result["serverInfo"]["name"] == "imagegen"
...
# --- 音声合成 ---------------------------------------------------------
def test_tool_list_includes_speech_tools():
    names = {tool["name"] for tool in mcp_server.tool_definitions()}
    assert {"synthesize_speech", "list_voices"} <= names


def test_synthesize_speech_saves_a_file(tmp_path):
    result = call(
        "tools/call",
        {
            "name": "synthesize_speech",
            "arguments": {"text": "こんにちは", "provider": "beep", "out": str(tmp_path)},
        },
    )["result"]
    ...
    assert "秒" in text
    assert "プレースホルダ" in text  # 読み上げでないことを必ず伝える


def test_synthesize_speech_requires_text():
    result = call("tools/call", {"name": "synthesize_speech", "arguments": {"provider": "beep"}})
    assert result["result"]["isError"]


def test_list_voices_returns_the_placeholder_tones():
    result = call("tools/call", {"name": "list_voices", "arguments": {"provider": "beep"}})
    assert "mid" in result["result"]["content"][0]["text"]


def test_list_voices_explains_a_missing_key():
    result = call("tools/call", {"name": "list_voices", "arguments": {"provider": "elevenlabs"}})
    assert "ELEVENLABS_API_KEY" in result["result"]["content"][0]["text"]
