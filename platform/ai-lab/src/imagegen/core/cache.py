...
def cache_dir() -> Path:
    """キャッシュ置き場（IMAGEGEN_CACHE_DIR で変更可）。"""
    base = os.environ.get("IMAGEGEN_CACHE_DIR") or (Path.home() / ".cache" / "imagegen")
    path = Path(base)
...
def default_ttl() -> int:
    """既定のTTL（IMAGEGEN_CACHE_TTL で変更可。0 でキャッシュ無効）。"""
    raw = os.environ.get("IMAGEGEN_CACHE_TTL")
