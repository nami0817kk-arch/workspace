...
def write_credits(saved: list[tuple[Asset, Path]], dest_dir: str | Path) -> tuple[Path, Path]:
    ...
    lines = [
        ...
        "利用条件は各ライセンスに従ってください（CC BY 系は表示が必須、CC BY-SA は継承も必要）。",
        "",
        "> **ライセンスと被写体の権利は別物です。**",
        "> CC ライセンスが許可しているのは撮影者の著作権だけで、写っている人物の",
        "> 肖像権・パブリシティ権、写り込んだロゴの商標権は別に扱われます。",
        "> 広告や商品など、商用の宣伝に使う場合は別途許諾が要ることがあります。",
        "",
...
def max_workers() -> int:
    """横断検索・一括ダウンロードの並列数（IMAGEGEN_MAX_WORKERS で変更可）。"""
    from .config import get_env

    try:
        return max(1, int(get_env("IMAGEGEN_MAX_WORKERS") or 4))
    except ValueError:
        return 4
