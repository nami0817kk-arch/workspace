...
レシピを毎週回すワークフローは、モノレポ統合（2026-09-01）の再配線で**未移植**。
現在このリポジトリに定期実行の仕組みは無い。

復活させるなら、ルートの `.github/workflows/` に `paths:` を `platform/ai-lab/**` へ
絞ったワークフローとして作り直す（`.github/workflows/ailab-tests.yml` が先例）。
統合前は次の構成だった:
...
既定は月曜9時JST・ドライラン。Actions タブの「Run workflow」から手動実行もでき、
そのときはレシピ名と送信可否をその場で選べた。生成物は成果物（artifact）として14日間残る。
