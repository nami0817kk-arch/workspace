...
| Phase | 状態 |
|---|---|
| 0. 基盤（`core/` + `connectors/` + `imagegen connectors` / `doctor`） | **完了** |
| 2. 出力先（GitHub） | **完了**（`imagegen publish`。Slack / Discord は当面不要と判断） |
| 1. 取得先追加（Iconify / Unsplash / Pexels） | **完了** |
| 生成モデル追加（Replicate / Hugging Face） | **完了** |
| 3. 情報収集（RSS / GitHub / Qiita） | **完了** |
| 4. パイプライン（YAMLレシピ） | **完了** |
| 5. MCP化 | **完了** |
| 6. 音声合成（speech） | **完了**（`imagegen say`。VOICEVOX / OpenAI / ElevenLabs / beep） |
| 7. 一次情報（官公庁の公式API） | **完了**（`imagegen feed --source edinet\|estat`） |

実装の使い方と新しい連携先の足し方は [connectors.md](connectors.md)。

---

`imagegen` を「画像生成＋フリー素材取得ツール」から、**外部サービス連携の共通基盤**へ広げるための計画。
...
| | MCPコネクタ（Claudeに接続） | imagegen の自前コネクタ（Python） |
|---|---|---|
...
`imagegen` に実装するのは「**スクリプトから何度も同じ手順で叩きたいもの**」と
...
src/imagegen/
  ...
  generation.py        # 後方互換シム（imagegen.generation.generate は今も動く）
  illust.py          # 後方互換シム
  cli.py
...
class Connector(ABC):
    ...
    def check(self) -> CheckResult: ...       # 実際に1回叩いて疎通確認（imagegen doctor）
```
...
| `SynthesizeSpeech` | `synthesize(text, *, voice, speed) -> SynthesizedSpeech` | VOICEVOX, OpenAI, ElevenLabs |

CLIは「コネクタ名」ではなく「**能力**」でディスパッチする。
`imagegen publish x.png --to slack` は `PublishFile` を持つコネクタだけを候補にする。

### 横断的に基盤へ入れるもの

- **再試行**: 429/5xx は指数バックオフ（`Retry-After` があれば従う）。上限3回。
- **レート制限**: コネクタ宣言のトークンバケット。Unsplash 50req/h のような枠を自動で守る。
- **キャッシュ**: 検索結果を `~/.cache/imagegen/` に短期キャッシュ（既定15分、`--no-cache` で無効）。
  同じ検索で無駄にAPI枠を消費しないため。
- **エラーの言い換え**: 401→「キーが違う」、403→「権限/枠」、429→「待って再実行」を日本語で出す。
  現状の「ネットワークエラー: HTTPSConnectionPool(...)」のような生の例外は出さない。
- **秘密情報**: `.env` のみを正とし、OAuthトークンは `~/.imagegen/credentials.json`（chmod 600）。
...
| 連携先 | 認証 | 実装コスト | 備考 |
|---|---|---|---|
| **GitHub** | PAT | 小 | **実装済み**。生成物をリポジトリへコミット（`imagegen publish`） |
...
報道や要約ではなく、提出書類・統計表そのものを引く。

| 連携先 | 認証 | 価値 |
|---|---|---|
| **EDINET**（金融庁） ✅ | 無料キー | 有報・四半期・大量保有報告書。株の分析で「報道ではなく書類で確かめる」入口 |
| **e-Stat**（政府統計） ✅ | 無料キー | 各省庁の統計表。数字の出どころを一次資料に置ける |

**入れないもの**と、その理由。

| 対象 | 理由 |
|---|---|
| TDnet（適時開示） | 公式APIが無い。HTMLスクレイピングはしない方針のため対象外 |
| 気象庁・arXiv・国会図書館 | Atom / RSS で配信されており、既存の `rss` コネクタでそのまま読める。二重に作らない |
...
| Phase | 内容 | 完了条件 | 目安 |
|---|---|---|---|
| **0. 基盤** ✅ | `core/` 新設、既存7コネクタを移行、`imagegen connectors` / `imagegen doctor` 追加 | 完了。契約テストで新コネクタの実装漏れも検出する | 完了 |
| **2. 出力先** ✅ | GitHub（`imagegen publish`、既定ドライラン） | 完了。Slack / Discord は要望が出たら追加 | 完了 |
| **1. 取得先追加** ✅ | Iconify（キー不要）、Unsplash、Pexels | 完了。`imagegen search "cat"` が6サイト横断 | 完了 |
| **生成モデル追加** ✅ | Replicate / Hugging Face | 完了。`imagegen gen --provider replicate --model owner/name` | 完了 |
| **3. 情報収集** ✅ | RSS / Atom、GitHub リリース、Qiita | 完了。`imagegen feed "対象" --source rss\|github\|qiita` | 完了 |
| **4. パイプライン** ✅ | YAMLレシピ。`imagegen run <レシピ>` | 完了。[recipes.md](recipes.md) |
| **5. MCP化** ✅ | `imagegen mcp`（stdio / JSON-RPC、依存追加なし） | 完了。[mcp.md](mcp.md) |
...
2. **書き込み系は既定でドライラン。** `imagegen publish` は `--yes` を付けるまで送信内容を表示するだけ。
   外部への投稿は取り消せないため。
...
| リスク | 対策 |
|---|---|
| 連携先を増やすほどAPI仕様変更で壊れる | コネクタは50行以内に保ち、壊れたものは `imagegen doctor` で即座に検出できるようにする |
| OAuth（Drive等）の実装が重い | Phase後半に回す。当面はキー1本で済むサービスを優先 |
| APIキーの管理が煩雑 | `imagegen doctor` が「どのキーが無くて、どこで取れるか」をURL付きで案内 |
| 課金APIの使いすぎ | Phase 4以降で `imagegen usage` に生成回数と概算コストを記録 |
