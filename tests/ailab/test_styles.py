...
def test_user_styles_can_be_added(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("AILAB_STYLES_FILE", str(path))
...
def test_generate_uses_the_style(tmp_path, monkeypatch):
    ...
    image = imagegen.generate("猫", provider="local", size="64x64", style="flat")[0]
    assert "フラットイラスト" in image.prompt  # プロバイダへ渡ったプロンプトが残る
