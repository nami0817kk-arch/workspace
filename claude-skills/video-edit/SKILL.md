...
| ファイル名が選手名でも、**本人と断定できない**（代表チームの集合写真など） | 下の「人物は機械で確かめる」を通す。確認できないなら顔を使わず、クラブカラー＋名前のカードにする |
| 被写体が**未成年** | 使わない |
...
### 人物は機械で確かめる（ファイル名は根拠にならない）

**ファイル名に選手名が入っていても、被写体がその人とは限らない。**
2026-09-03 の実測で、`Ayase Ueda` を名前に含む写真3枚を調べたところ:

| ファイル | Commons の depicts（被写体の構造化データ） |
|---|---|
| `...53679115528.jpg` | **Joris Kramer**（別人が明記されていた） |
| `...(Ayase Ueda).jpg` | 人物の指定なし |
| `...(Ayase_Ueda_02).jpg` | 人物の指定なし |

**`check_licenses.py` は3枚とも OK を返していた。** ライセンスは写真の著作権しか
見ないので、被写体が誰かは分からない。目視でユニフォームが対戦相手のものだと
気づかなければ、誤った人物を本人として公開していた。

Commons は `depicts`（P180）に、写っているものを Wikidata の ID で持っている。
これを引いて照合すれば機械で確かめられる。youtube PJT に実装がある:

```bash
cd C:/Users/なみ/dev/workspace/projects/youtube-video-creation
python -m src.cli subject assets/images/<フォルダ> 上田綺世 "Ayase Ueda"
```

日本語名と英語名の**どちらか**が当たれば本人とみなす（Wikidata のラベルは
言語ごとに別で、英語だけで照合するとメッシのように日本語ラベルしか持たない
項目を取りこぼす）。

**設計の要点: 指定が無いものを「本人ではない」と断定しない。**
「確かめられない」として扱い、使わない。誤った人物を出すより、使わないほうがよい。

**これを通しても目視は省けない。** Enzo Fernández では4枚とも本人と確認できたが、
実際に使えたのは1枚だけだった。残り3枚は遠景で、誰が本人か画面から判別できない。
**「写っている」ことと「主役として使える」ことは別。**
