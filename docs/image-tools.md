...
APIキーが設定されているプロバイダ・素材サイトに `OK` が付く。
...
`--provider auto`（既定）は openai → gemini → stability → local の順に、
使えるものを選ぶ。どのAPIも有料なので、試作中は `local` で十分なことも多い。
...
items = illust.search("cat illustration", source="openverse", limit=5)
illust.download_all(items[:2], "output/illust")
```
