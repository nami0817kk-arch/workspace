...
`config/project.yaml` の `titles` で、冒頭と章の変わり目にタイトルカードを差し込む。
0 にすると出さない。

| 項目 | 内容 |
|---|---|
| `intro` | 冒頭タイトルの秒数（既定 2.6） |
| `chapter` | 章タイトルの秒数（既定 1.4）。冒頭タイトルの直後になる1つ目の章には入らない |
| `outro` | 最後に出すカードの秒数（既定 3.0） |
| `fade` | 出入りのフェード秒 |

台本の frontmatter で中身を指定する。

```markdown
---
title: 【サッカーニュース】...           # 概要欄・YouTubeのタイトル
intro_title: アルバレスはどこへ？        # 冒頭カードに出す短いタイトル
intro_label: 海外サッカー ニュース       # タイトルの上に出す小さなラベル
date: 2026年8月29日                     # カードと画面右上に出る日付
outro_title: 期限まで、あと数日          # 最後のカードの見出し
outro_sub: 動きがあり次第、続報をお伝えします
---
```

`intro_title` を省略すると `thumbnail_title`、それも無ければ `title` を使う。

**カードのぶんは音声に無音が挿し込まれる**ので、映像と音声はずれない。
字幕とチャプターの時刻も、実際の発話開始位置に自動で追従する。
...
| `background_zoom` | 静止画背景をゆっくり寄せる倍率（既定 1.12）。1.0 で止めたまま |
| `fps` | アニメーション部分の描画レート |

`background_zoom` を 1.0 より大きくすると、静止画の背景をシーンの長さに合わせた
クリップに変換してから使う。止まった絵が続かないので動画らしくなる。
作ったクリップは `assets/backgrounds/.motion/` にキャッシュされ、2回目以降は再利用する。
...
**投稿は取り返しがつきません。** `--dry-run` を付けると、認証も通信もせずに
何が送られるかだけを出します。

```bash
python -m src.cli upload output/20260831_morning --dry-run
```

```
■ 投稿の中身　output/20260831_morning
  タイトル　【速報】トッテナムがアルバレス獲得　（17字）
  公開設定　private
  動画　　　output/20260831_morning/video.mp4　38.2MB
  サムネ　　output/20260831_morning/thumbnail.png
  タグ　　　9個 / 合計86字
  　　　　　サッカー / 海外サッカー / サッカーニュース / プレミアリーグ / ...
  概要欄　　612字　先頭: トッテナムがアルバレス獲得
```

タイトル100字・概要欄5000字・タグ合計500字の制限もここで見ます。
超えていれば `×` を出して止まるので、投稿の途中で弾かれることがありません。

タグは `description.txt` には入っていないので、ビルドが書いた `script.json`
から取ります（**以前は投稿時にタグを送っていませんでした**）。
...
同じ台本から、1080×1920 のショートを書き出せます。

```bash
python -m src.cli short scripts/YYYYMMDD_match.md
python -m src.cli short scripts/YYYYMMDD_match.md --section 数字で見ると
```

**切り方は「冒頭 + 1つの節」**です。60秒に全部は入りませんし、入れたところで
本編を見る理由がなくなります。冒頭で何の話かを言い、1つだけ掘って、続きは本編へ送る。

- 節を指定しなければ、冒頭の次の節を使う
- 尺に収まらなければ、**掘る節の後ろのセリフから落とす**（冒頭は削らない）
- 縦は横幅が狭いので、テロップと見出しの字を自動で小さくする
- 60秒を超えたら終了コードが1になる。`--section` で短い節を選ぶか台本を削る

## BGM の使い分け

台本の頭の札で曲が変わります。**【速報】と【詳報】で同じ曲が流れると、
どちらも同じ温度に聞こえる**ためです。

| 札 | 曲 | 中身 |
|---|---|---|
| 【速報】【悲報】 | `bgm_breaking.wav` | 112BPM、半音進行を混ぜて落ち着かなくする |
| （札なし）【朗報】 | `bgm_loop.wav` | 96BPM、Am-F-C-G |
| 【詳報】 | `bgm_calm.wav` | 80BPM、長調で動きを減らす |

frontmatter に `bgm:` を書けばそちらが優先されます。自分の音源を
`assets/audio/` に置く場合もここで指定します。

どれも自前で合成しているので権利の問題は出ませんが、**素材を用意できるなら
そちらのほうがよい**です。3層（パッド・アルペジオ・低音）の簡素な作りです。
