...
<h2>「いま条件がそろっている商品」の出し方</h2>
<p>当サイトは値下がり・最安値圏・ポイント・送料を別々の一覧で出していますが、
読む側は「結局どれか」を知りたいはずです。そこで、満たした条件を点にして
足し合わせた順に並べたページを用意しました。買うべきかどうかは決めません。
どの条件がいくつ満たされたかを出すだけです。</p>
<p>点の付け方は次のとおりで、すべて四則演算です。各商品の一覧には内訳を
そのまま表示しています。</p>
<ul>
<li>最安値への近さ（最大40点）記録した中の最安値との差が0%なら40点、30%以上離れていれば0点</li>
<li>ポイント込みの下げ幅（最大25点）実質価格が前回より20%下がっていれば満点</li>
<li>価格の下げ幅（最大20点）価格が前回より20%下がっていれば満点</li>
<li>送料無料（10点）送料込みで出品されている場合</li>
<li>値動きの多さ（最大5点）記録期間に5回以上動いていれば満点</li>
</ul>
<p>記録が7日に満たない商品と、在庫切れの商品は対象から外しています。
前者は最安値を名乗れず、後者は買えないためです。</p>

<h2>ポイントを含めた「実質価格」について</h2>
<p>楽天市場では、値引きが価格そのものではなくポイント倍率で行われることが多くあります。
当サイトの記録では、価格が1日で5%以上下がった商品が17件だった日に、ポイント倍率を
含めた実質価格では376件が下がっていました。価格だけを見ていると、その日の値引きを
取り逃すことになります。</p>
<p>そのため当サイトは「価格 −（価格 × ポイント倍率 ÷ 100）」を実質価格として併記し、
<a href="../points/">ポイント込みで安くなった商品</a>を別に出しています。
ポイントは現金ではなく、実際の付与数は SPU や会員ランクによっても変わるため、
表示しているのはあくまで目安です。価格そのものの履歴と判定も従来どおり残しています。</p>
"""
