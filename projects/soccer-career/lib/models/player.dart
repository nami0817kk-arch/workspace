...
class Player {
  ...
  ///
  /// 育てた結果として別の選手になっていることがある——守備を極めた
  /// 中盤の選手は、中盤の重みで測ると総合力が落ちるが、守備の選手として
  /// 測れば高い（実測で、中盤の選手を守備一本で育てるとピークが
  /// 75.4 → 69.4 に落ちた）。**それは間違った育て方ではなく、
  /// 別の選手になったということ**なので、そう言ってやる。
  ///
  /// 適性（`Aptitude.canConvert`）のあるポジションだけを見る。
  /// 今より [Formulas.convertGain] 以上高くなければ null。
  Position? get suitedPosition {
    if (aptitude.isUnknown) return null;
    Position? best;
    var bestOverall = overall + Formulas.convertGain - 1;
    for (final p in Position.values) {
      if (p == position || !aptitude.canConvert(p)) continue;
      final value =
          attributes.overallFor(p) -
          aptitude.penaltyFor(p, factor: traits.aptitudeFactor);
  ...
  int effective(Detail detail) =>
      (attributes.detail(detail) + physique.bonusFor(detail))
  ...
  factory Player.fromJson(Map<String, dynamic> json) {
    ...
    final physique = Physique.fromJson(
      json['physique'] as Map<String, dynamic>?,
    );
    return Player(
      ...
      potential:
          json['potential'] as int? ?? (attributes.overallFor(position) + 8),
      ...
      physique: physique,
      setPieces: SetPieceSkills.fromJson(
        json['setPieces'] as Map<String, dynamic>?,
      ),
