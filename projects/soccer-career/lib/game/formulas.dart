...
class Formulas {
  ...
  /// 途中出場のふつうの試合。
  ///
  /// **1 にしてはいけない。** 1局面だと、その1つの成否がそのまま
  /// 試合の評価点になる——**コイン投げ1回**で決まる。実測（`early_sim`）で
  /// 駆け出しの「評価 6.0未満」が 先発 19% 対 途中出場 **44%**（ST）と
  /// 倍以上開いていた。最初の15節は 3〜6割が途中出場なので、
  /// 新規の人はそこで「何をしても下手」に見える。
  ///
  /// **倍率（`ratingScale`）では直らない。** 基準点が 6.0 なので、
  /// 掛け算では 6.0 をまたぐ割合が1も動かない（測って外した）。
  /// 直せるのは標本の数——2 にして 44% → 22% になり、先発と揃った。
