...
class MatchInProgress {
  ...
  ///
  /// 重い試合は6局面、ふつうの試合は2局面。割り戻さないと、
  /// 重い試合に出ただけで評価点が跳ね、薄い試合に出ると下がる
  /// （＝出た試合の重さが、そのまま平均評価になってしまう）。
  /// 途中出場が先発より軽いことは、これまでどおり残す。
  ///
  /// **1 で止める案は、測って外した。** 足りないぶんを割り増さない形に
  /// すると平均だけ下がり、**6.0未満の試合の割合は 1 も動かなかった**
  /// （基準点が 6.0 なので、倍率を掛けても 6.0 をまたぐ割合は変わらない）。
  /// 振れ幅の問題は倍率ではなく**局面の数**の側にある。
