...
final List<_GuideSection> _sections = [
  const _GuideSection('1週間の流れ', [
    ...
    '練習は一度決めるとそのまま続く。今なにを選んでいるかは'
    '「試合」タブの試合へボタンのすぐ上に出ているので、'
    'そこから直接変えられる（居残りも同じところで決められる）。',
  ...
  const _GuideSection('試合で選ぶ', [
    ...
    'その％が何でできているかは、画面に出ている。局面の上に並ぶのは'
    '「どの手を選んでも同じだけ効くもの」（相手の格・コンディション・気持ち）、'
    '手ごとに付くのは「その手にだけ効くもの」（特性・覚えた個人技・自分の型・'
    '相手の戦い方・相方・逆足）。手を選ぶときに見るのは後者。',
    'ゴールやアシストの手には「ゴール○%」も出る。手が通っても決まるとは限らない。'
    '枠を捉えても止められるのがサッカーで、そこを分けてある。',
    '終盤（75分以降）に同点以下から決めた得点は、評価点が重く付く。',
    '終盤の局面は、そのときのスコアで中身が変わる。追いかけているなら'
    'リスクを取る局面、リードしているなら時間の使い方を問う局面が来る。'
    '2点差で追う展開は、もう少し早い時間から勝負になる。',
  ...
  _GuideSection('評価点と出場機会', [
    ...
    '荒い手は、失敗すると警告を受けることがある。気性が荒いほど笛が鳴る。'
    '今季5枚で1試合の出場停止、退場なら2試合。警告はシーズンをまたぐと消える。',
    '「止めるための反則」は、選べば必ず警告になる代わりに、'
    'これから入るはずだった失点を1つ消す。取るか取らないかを決める手。',
    '調子が良くても、疲れていれば休まされることがある。'
    '長い離脱から戻った直後も、まずはベンチからになる。',
    '次にどの立場で出られそうかは、「試合」タブの次節のところに出る。'
    '直近の平均・監督の信頼・外れているぶんの情けを足した数字と、'
    'その線が並ぶので、落ちた理由が分かる。',
  ...
  _GuideSection('伸ばす', [
    ...
    '練習ではその項目が優先して伸び、試合の成長もそこに寄る。'
    '**伸びる量は変わらない**——どこに乗るかだけが変わる。'
    '選んだ項目を使う手には、試合中に「重点」と出る。',
    '1つの項目を78まで上げると、その項目の個人技を覚えることがある。'
    '「育てる方向」には、あといくつで届くかが出ている。',
    '「育成」タブの一番上に、今週なにをするかの手がかりが出る。'
    '離脱中か、疲れているか、次の相手が何を潰しに来るか、'
    '監督の期待にどれだけ届いていないか。押せば練習をその場で切り替えられる。',
  ...
  const _GuideSection('選手を作る', [
    '名前・ポジション・年齢・身体・能力の割り振り・代理人を決めて始める。'
    'ポテンシャルと特性は選べない。始めてから分かるのがキャリアもの。',
    'サイドバックとウイングは、左右のどちらに立つかを決める。'
    '利き足と同じ側なら逆足で対応する局面が減り、逆サイドなら増える。'
    'その代わり逆サイドは、内へ切り込んで利き足でシュートを打てる。',
    '身長は競り合いに、体重は当たりの強さとキレの入れ替えに効く。'
    '練習では動かないので、ここで決めた体格は一生ついてくる'
    '（オフの肉体改造で少しだけ動く）。',
    '能力はポジションの基準値から±6まで動かせる。'
    '増やしたぶんはどこかを削るので、総合力は変わらない。'
    '何を得意にするかだけを決める。',
    '出身国で、始めるリーグと代表と、外国人としての扱いが決まる。'
    '格の高い国は水準も高いぶん、上がるのは難しい。',
    '肌・髪型・髪の色・背番号も決められる。試合の判定には効かない。'
    'ユニフォームは所属クラブの色になるので、移籍すると見た目も変わる。',
  ]),
