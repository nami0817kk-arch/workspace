...
- **1シーズン目の終わりから出る**（`adsFromSeason` = 1）。出る場所が季末しか
  無いので、無料にした季はそのまま機会から消える（実測 **18.6回/キャリア**）。
- 前の広告から **3分**（`adInterval`）あいていること。シーズンは自動で
  飛ばせるので、置かないと連発する。**手で進める人には当たらない。**
- **引退した季も数える。** キャリアで最後の1回になる。
- 買った人には出さない。

## 星（評価）

`lib/feedback/review_prompt.dart`。**広告や課金とは別の話だが、入手率に
一番効くのがここ**——星が0のままだと、見つけてもらっても入手されない。

3季を終えてから、良い季（目標達成・優勝・昇格）にだけ、キャリアで1度だけ頼む。
**広告を出した回は見送る**（`showSeasonAd` が出せたかを返すので、それを見る）。
コンソール側の作業は無い。
