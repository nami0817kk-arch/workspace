"""IPA の解答例 PDF から、午前問題の正解表を取り出す。

問題冊子のほうは文字層を持たないスキャン画像なので、ここでは扱わない
（2026-09-24 に令和7年度・令和5年度の応用情報で確認。総文字数 0）。
解答例 PDF は文字層があり、問番号・正解・分野が素直に取れる。
...
# 正解として認めるのは多肢選択の記号だけ。記述式（午後）はここでは扱わない。
_CHOICES = "アイウエ"
