...
class SaveGame {
  ...
  // ここは以前セーブの大半を占めていた。**2026-09-03 に削減済み**(55d09f4)。
  //
  //   着手前: セーブ全体 1,090KB / うち otherDivisionLeagues 860KB (79%)
  //   削減後: 開始時 1,092KB → 415KB / シーズン遷移後 1,136KB → 444KB
  //
  // 他ディビジョンは順位表と、そこから決まる昇降格にしか使わない。そこで
  // 選手データを保存せず、強度(`Team.retainedOverall`)だけを残している。
  // 書き出しの切り替えは `Team.toJson` / `League.toJson` の includePlayers で、
  // `SaveGame.toJson` が otherDivisionLeagues にだけ false を渡す。
  //
  // 削減の見積もりについて、後から入る人向けの注意を残す。
  // **着手前の「1/5 になる」という推定は外れた。** 落ちるのは選手データだけで、
  // 38節ぶんの日程と試合結果は5ティア分そのまま残るため、そこが残りを占める。
  // 数字を出すときは推定でなく測ること(測り方でも変わる。CI 上で1シーズン
  // 消化した合成セーブでは 2,195KB → 1,515KB の31%減だったが、実アプリの
  // 開始時・遷移後で測ると上記のとおり6割減になる)。
  //
  // 「選手を削るだけ」では済まなかった点。壊れ方の対処が入っているので、
  // ここを触るときは巻き戻さないこと。
  //
  //   1. Team.overallRating は players から計算しており、空にすると 0 を返して
  //      BackgroundMatchEngine のチーム強度が壊れる。players が空のときは
  //      retainedOverall を返すようにしてある。
  //   2. 昇格して自ディビジョンに入ったチームは MatchEngine.simulate で
  //      分単位・選手ごとに戦うため選手が要る。`_ensureSquadsForActiveTier`
  //      (game_state_season.dart)が保持強度に見合うスカッドを生成する。
  //      その結果、昇格したチームは「昇格を勝ち取った実際のスカッド」ではなく
  //      新規生成されたものになる。他ディビジョンの選手を見る画面は無いので
  //      利用者は気づけないが、内部的には別物になる。将来そういう画面を
  //      作ると、この矛盾が表面化する。
  //   3. セーブ形式が変わる。`Team.fromJson` の players を null 許容にして
  //      あり、旧形式(選手あり)はそのまま読める。読み込み後の最初の保存で
  //      新形式になる。この一方向の移行は消さないこと。
  //
  // 検証は test/save_size_test.dart。旧形式が読めることは、実装を入れる前に
  // 単独で緑を確認してある。
