...
各コマンドの中身は見ない（ネットワークと API キーが要る）。配線の確認まで。
...
def test_invalid_choice_for_category_exits_with_code_2(monkeypatch):
    """--category は選択肢が決まっている。範囲外は終了コード2。"""
    monkeypatch.setattr(sys, "argv", ["main.py", "run", "--category", "そんな分類は無い"])
    with pytest.raises(SystemExit) as e:
        main.main()
    assert e.value.code == 2
