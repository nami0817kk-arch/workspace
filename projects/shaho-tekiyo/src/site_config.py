...
kabu-agari-ranking と同じ理由（`projects/kabu-agari-ranking/src/site_config.py`）。
"""
import os

# 末尾のスラッシュは付けない（URL の組み立て側で付ける）。
SITE_URL = os.environ.get("SHAHO_SITE_URL", "https://shaho.dailyquarry.com").rstrip("/")

# Search Console の所有確認タグ。同じ Google アカウントならドメインが変わっても
# 同じ値（docs/public-identity.md で確認済み）。
SEARCH_CONSOLE_TOKEN = os.environ.get(
    "SHAHO_SEARCH_CONSOLE_TOKEN", "JLPv6DMxv7h91q8Hzvo-YfdQx6mz-_zZ-MhHaNzPs4c"
)

# 公開名義と連絡先。**個人名は出さない**（屋号で通す）。
OWNER = os.environ.get("SHAHO_OWNER", "つるはし社")
