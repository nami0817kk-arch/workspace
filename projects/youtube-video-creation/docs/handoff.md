...
**Sky の記事IDが1本の並びになっていない。** `research/freshness.yaml` に
`skysports.com` の水準を記録しているが、実測で同じ日に `13579158` と
`13176154` の両方が入った。ニュース記事(`/football/news/...`)と
ライブブログ(`/football/live-blog/...`)でIDの並びが別になっているためらしい。

`freshness` は「そのサイトで最後に見た番号」から記事の新しさを推定する仕組みで、
番号の並びが2本あると推定が当てにならない。

- **いまのところ実害は小さい。** Sky はフィードから取るので正確な時刻が付く。
  推定が要るのは、検索で拾った Sky のURLだけ
- 直すなら「サイト単位」ではなく「サイト＋記事の種類」で水準を持つことになる。
  ただし他のサイトでも同じ構造なのかを測っていないので、まだ決められない

## 変わらないこと

ネットワークが開いても、次の2つは変わらない。

- **Xで「見つける」のは、いまも検索か有料API。** 本文のほうは 2026-08-31 に解決した。
  publish.twitter.com/oembed が認証なしで全文と正のハンドルを返すので、
  `x <URL>` で引ける（src/xembed.py）。ただし**URLを既に持っている前提**の
  しくみなので、索引の1〜2日の遅れは埋まらない。速報には使えないまま。
  `api.x.com` は有料プランのトークンが要る（`X_BEARER_TOKEN` を環境変数で渡す設計。
  **ファイルに書かないこと**）
- **AI要約は使わない。** 検索エンジンの要約が事実でないものを混ぜてくるのは
  ネットワークの制約とは無関係。本文が読めるようになったら
  「要約に頼らず原文を引く」を徹底する方向に効く

## APIの枠を使い切った（2026-09-09）

公開済みショート35本のサムネを縦に貼り替えている途中、**10本目で
`quotaExceeded`（403）**に落ちた。

**何の枠かは測って確かめた。**15分後に叩き直しても、
**コスト1の読み取り（`videos.list`）まで同じ理由で弾かれる。**
だからサムネの連続制限（429）でも、瞬間的に叩きすぎたのでもなく、
**YouTube Data API v3 のプロジェクト全体の枠**が尽きている。

### **「1日10,000」は誤り。ユーザーの指摘で気づいた**

最初「10,000の枠を使い切った」と説明したが、**数が合わない。**
この枠の日（太平洋時間 09-08 = 日本時間 09-08 16:00〜09-09 16:00）に、

| 操作 | 回数 | 公表コスト | 小計 |
|---|---|---|---|
| videos.insert | 64 | 1,600 | 102,400 |
| thumbnails.set | 59 | 50 | 2,950 |
| videos.update | 54 | 50 | 2,700 |
| search.list | 約9 | 100 | 約900 |
| commentThreads.insert | 14 | 50 | 700 |
| 読み取り | 102 | 1 | 102 |

**10万を超えている。**既定の10,000なら動画6本目あたりで止まっていたはずで、
実際には64本通った。**このプロジェクトの枠は10,000ではなく、もっと大きい。**
`src/quota.py` の `DAILY = 10000` は Google の既定値を書き写しただけで、
一度も確かめていなかった。**それを実測のように話した。**

正しい数字が出るのは Google Cloud のコンソールだけ
（APIとサービス → YouTube Data API v3 → 割り当て）。APIは残量を返さない。

### 上限を調べた（2026-09-09、Gemini にも確認）

**コンソールは開けなかった。**プロジェクトは `graceful-byway-507707-v7`
（`secrets/client_secret.json` に記載）。リモートデバッグ用の Chrome は
別の Google アカウントでログインしていて、`resourcemanager.projects.get`
が無く弾かれる。**ここはユーザーが自分で開くしかない。**

```
https://console.cloud.google.com/apis/api/youtube.googleapis.com/quotas?project=graceful-byway-507707-v7
（または IAM と管理 → 割り当て → YouTube Data API v3 → Queries per day）
```

**代わりに、こちらの控えから下限を出した。**`posted.json` の投稿時刻を
太平洋時間で束ねる。

| 枠の日（太平洋時間） | 投稿本数 | 1,600換算 |
|---|---|---|
| 09-05 | 8 | 12,800 |
| 09-06 | 34 | 54,400 |
| 09-07 | 16 | 25,600 |
...
既定の10,000なら**1日6本で止まる。**58本通っている。
したがって**このプロジェクトの枠は少なくとも 92,800 以上**。

Gemini の回答（2026-09-09）:

- 「既定の10,000のままでは起こり得ない。**割り当てが引き上げられていると
  考えるべき**」
- リセットは太平洋時間の深夜0時。**夏（PDT）は日本時間16:00、
  冬（PST）は17:00**
- **残量を返すレスポンスヘッダもエンドポイントも無い。**自前で数えるしかない
- 上限そのものはコンソールの「Queries per day」で見える

### 分かったこと

- **枠を食ったのはサムネではなく、64本の投稿。**貼り替え（50×35=1,750）は
  最後のひと押しにすぎない
- `thumbnails.set` は単独で叩くと50かかる。台帳では0として数えていた
- `search.list` は1回100。参考チャンネル探しで9回＝900使った
- 枠切れは待っても直らない。太平洋時間の深夜0時＝日本時間16時まで戻らないので、
  `quotaExceeded` を見たらその場で止める
- **`quota` の表示で「もう出せない」と判断しない。**
