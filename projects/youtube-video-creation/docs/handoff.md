...
## APIの枠を1日ぶん使い切った（2026-09-09）

公開済みショート35本のサムネを縦に貼り替えている途中、**10本目で
`quotaExceeded`（403）**に落ちた。サムネの制限（429）ではなく、
**1日10,000のリクエスト枠**のほう。

分かったこと。

- **`thumbnails.set` は単独で叩くと50かかる。**台帳では0として数えていた
  （投稿1本ぶん270に含めていたため）。**投稿せずに貼り替える使い方が
  数えられていなかった。**今日は17本ぶん＝850が台帳の外にあった
- **`search.list` は1回100。**参考チャンネルを探すのに8回叩いて800使った。
  調べもので気軽に使うと、そのぶん投稿の枠が減る
- 枠は**太平洋時間の深夜0時＝日本時間16時**に戻る。待つ以外に手は無い。
  3分おきの再試行は無駄なので、`quotaExceeded` を見たらその場で止める

**35本の一括作業を始める前に `quota` を見ていなかった。**見ていれば
20,782/10,000 と出ていた。数える仕組みは6日前からあるのに、使っていない。
