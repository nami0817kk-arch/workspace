...
**動画を64本上げたこと。**それだけで全体の93%。

| 操作 | 回数 | 公表コスト | 小計 |
|---|---|---|---|
| **videos.insert** | **64** | 1,600 | **102,400** |
| thumbnails.set | 76 | 50 | 3,800 |
| videos.update | 54 | 50 | 2,700 |
| search.list | 9 | 100 | 900 |
| commentThreads.insert | 14 | 50 | 700 |
| 読み取り | 102 | 1 | 102 |
| | | | **110,602** |

サムネの貼り替え（3,800）も検索（900）も、**合わせて4%**にしかならない。
最後のひと押しではあっても、原因ではない。

**疑った2つは、どちらも違った。**

- **Claude(Anthropic) のAPIではない。**エラーの本文が
  `{"domain":"youtube.quota","reason":"quotaExceeded"}` で、
  youtube.googleapis.com が返している
- **並行セッションでもない。**本体（`workspace/`）側の `research/quota.json` が
  今朝10:10に更新されていて疑ったが、**このファイルは git で追跡されている**。
  reflog を見たら 10:10 は `pull --ff-only origin master` だった。
  同時刻に走っていた別セッションは Site watch だけで、YouTube には触らない

### 枠の実際の値（再現から）

64本目の投稿が通った時点で1600換算の累計は10万を超えている。そのあと
サムネを10枚(500)貼れて、11枚目で落ちた。したがって
**枠はおよそ 106,000〜110,000**。既定の10,000ではなく引き上げ済みの値で、
正確な数はコンソールにしか出ない。

**実用的な目安: 1日に上げられる動画は65本前後**（110,000 ÷ 1,600 ≒ 68）。
本編とショートで1題材2本なので、**1日およそ32題材**が上限。
