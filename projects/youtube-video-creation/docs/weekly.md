...
ファイルの有無から進み具合を読み取って、**次に打つコマンドを1つだけ**出します。

```
■ 2026年8月30日 の進み具合
  ✓ 候補　research/20260830_candidates.yaml
  ✓✓✓ 朝の一報　取材メモ / 台本 / 書き出し
  ・・・ 昼の話題　まだ何もない
  ・・・ 夜のまとめ・深掘り　まだ何もない

次にこれを打つ:
  python -m src.cli plan --routine noon --write
```
...
`covered.yaml` は重複を止めるための記録ですが、同じものを数えると
**どこに偏っているか**が見えます。

```
■ 直近14日　2本（1日あたり 0.1本 / 目標 3本）
  08/30　■■□　2本
  08/29　□□□　0本

! 1日あたり0.1本です（目標3本）。本数を追うか、目標のほうを下げるか決めてください
! 1本も出していない日が13日あります
```

枠の偏り（朝ばかり）と、テーマの繰り返し（同じ選手を3回）も出します。
**目標に届かないときは、本数を追うか目標を下げるかを決める**ためのものです。
続けられない目標のまま走ると、質のほうが先に落ちます。
