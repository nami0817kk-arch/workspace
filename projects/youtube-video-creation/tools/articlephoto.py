...
def main() -> int:
    ...
    if agency:
        # **止める。逃げ道は置かない**（2026-09-25）。警告だけにしていたら、
        # 上田綺世のサムネに AFLO 配信の og:image を取り込んで、書き出しまで進んでいた。
        # 代理店は画像を機械照合して請求書を送る。媒体への削除要請とは別の話
        print(f"■ **{agency} の配信です。取り込みません。**代理店は画像を機械照合して"
              "請求書を送ります。別の候補（媒体自身の写真）か、別の記事を当たってください",
              file=sys.stderr)
        return 2
