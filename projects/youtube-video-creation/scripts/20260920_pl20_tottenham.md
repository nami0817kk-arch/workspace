---
title: 高井幸大のトッテナム。リーグの外からFAカップをつかんだクラブの結末
format: news
voice_min: 0.0
short_title: 高井幸大のトッテナム、リーグ外でFAカップを獲った結末
series: プレミアリーグチーム紹介
topic: トッテナム
thumbnail_line1: リーグの外から、FAカップ
thumbnail_line2: 高井幸大が入った、スパーズの始まり
thumbnail_tags: []
thumbnail_alt: []
thumbnail_points: []
thumbnail_note_red: ''
thumbnail_band_full: false
thumbnail_photos: []
thumbnail_crest_main:
- トッテナム
thumbnail_board: assets/stats/pl_tottenham_data_t.png
bg: assets/backgrounds/stadium.png
date: 2026年9月19日
intro_title: 高井幸大のトッテナム。リーグの外からFAカップをつかんだクラブの結末
intro_label: 海外サッカー ニュース
outro_title: 続報は次回お伝えします
outro_sub: チャンネル登録でお待ちください
description: '高井幸大のトッテナム。リーグの外からFAカップをつかんだクラブの結末


  この動画が答える問い: どんなクラブなのか


  ※各社の報道をもとにしています。クラブが発表した「確定」、

  報道機関が伝える「報道」、SNS段階の「未確認」、

  経緯の説明である「背景」を画面上で分けています。

  '
tags:
- サッカー
- トッテナム
- 高井幸大
- ダニー・ブランチフラワー
- プレミアリーグ
- 解説
- 海外サッカー
- サッカーニュース
sources:
- https://en.wikipedia.org/wiki/Tottenham_Hotspur_F.C.
- https://en.wikipedia.org/wiki/2026–27_Tottenham_Hotspur_F.C._season
- https://en.wikipedia.org/wiki/Danny_Blanchflower
- https://en.wikipedia.org/wiki/Roberto_De_Zerbi
- https://en.wikipedia.org/wiki/Harry_Kane
- https://en.wikipedia.org/wiki/Son_Heung-min
- https://en.wikipedia.org/wiki/Ledley_King
cards:
  cup1901_card:
    type: table
    title: 最初に立てた記録
    columns: &id001
    - ''
    - ''
    rows:
    - - 1901年
      - リーグ外のクラブとしてFAカップ優勝
    - - 以降
      - 同じ例は1つもない
  cup1901_3_card:
    type: table
    title: 最初に立てた記録
    columns: *id001
    rows:
    - - 1961年
      - 20世紀で最初の2冠
    - - 1963年
      - カップウィナーズカップ。英国勢で初
  cup1901_7_card:
    type: table
    title: 最初に立てた記録
    columns: *id001
    rows:
    - - 1972年・1984年
      - UEFAカップ。2種類目も英国勢で初
    - - 2025年
      - ヨーロッパリーグ。欧州は4つ目
  features_card:
    type: table
    title: このクラブの特徴
    columns:
    - ''
    - ''
    rows:
    - - プレーの色
      - つないで崩し、前から奪う
    - - 運営の型
      - 稼ぎ世界9位、追加で1億
    - - クラブの気質
      - 讃美歌から生まれた応援歌
  episode_card:
    type: table
    title: 雄鶏と拍車
    columns: &id002
    - ''
    - ''
    rows:
    - - ハリー・ホットスパー
      - 馬に拍車を蹴り込んだ人
    - - 拍車
      - 闘鶏の鶏にも付ける
  episode_2_card:
    type: table
    title: 雄鶏と拍車
    columns: *id002
    rows:
    - - 1909-10年
      - 元選手が35ポンドで雄鶏の像を作る
  legends_card:
    type: table
    title: 1990年以降の名選手
    columns:
    - 名前
    - 在籍
    - 残したもの
    rows:
    - &id003
      - ハリー・ケイン
      - 2009-2023
      - クラブ最多の280得点
  legends2_card:
    type: table
    title: 1990年以降の名選手
    columns:
    - 名前
    - 在籍
    - 残したもの
    rows:
    - *id003
    - &id004
      - ソン・フンミン
      - 2015-2025
      - EL優勝の主将
  legends3_card:
    type: table
    title: 1990年以降の名選手
    columns:
    - 名前
    - 在籍
    - 残したもの
    rows:
    - *id003
    - *id004
    - - レドリー・キング
      - 1999-2012
      - 一筋の主将
  manager_card:
    type: table
    title: 今季の監督
    columns:
    - ''
    - ''
    rows:
    - - 監督
      - ロベルト・デ・ゼルビ（イタリア）
    - - 就任
      - 2026年3月・2年目
    - - 前職
      - マルセイユ
  season_card:
    type: table
    title: プレミアリーグの結果
    columns:
    - 節
    - 相手
    - 結果
    rows:
    - - 第1節
      - ブレントフォード（アウェー）
      - 0対3
    - - 第2節
      - ニューカッスル・ユナイテッド（ホーム）
      - 0対2
    - - 第3節
      - ノッティンガム・フォレスト（アウェー）
      - 0対0
    - - 第4節
      - エヴァートン（ホーム）
      - 0対0
    - - 第5節
      - アストン・ヴィラ（ホーム）
      - 2対3
---

## オープニング
@bg: assets/backgrounds/stadium_トッテナム_in.png

キャスター: 北ロンドンから、**FAカップ8回**と**ヨーロッパリーグ3回**。
  telop: 北ロンドンから、**FAカップ8回**と**ヨーロッパリーグ3回**
  image: assets/stats/pl_tottenham_data_t.png
  no_telop: true
キャスター: 高井幸大のトッテナム。リーグの外からFAカップをつかんだクラブの結末。
  telop: 高井幸大のトッテナム。リーグの外からFAカップをつかんだクラブの結末
  se: assets/audio/se_pon.wav
  image: assets/stats/pl_tottenham_data_t.png
キャスター: 高井幸大は、2025年の夏に川崎フロンターレから来ました。
  telop: 高井幸大は、2025年の夏に川崎フロンターレから来ました
  image: assets/stats/pl_tottenham_data_t.png

## いま見る理由と基礎DATA
@bg: assets/backgrounds/stadium_トッテナム_in.png
@main: true

キャスター: 1961年のリーグとカップの2冠を主将として率いた、ブランチフラワーの言葉があります。
  no_telop: true
  source: 背景
  image: assets/stats/pl_tottenham_data_r1.png
ダニー・ブランチフラワー: 「サッカーは栄光のためにある。粋に、華やかにやるんだ」
  telop: ダニー・ブランチフラワー「サッカーは栄光のためにある。粋に、華やかにやるんだ」
  image: assets/stats/pl_tottenham_data_r1.png
キャスター: 高井幸大は**22歳**のセンターバック。いまは期限付きで、ベルギーのシント＝トロイデンにいます。
  no_telop: true
  image: assets/stats/pl_tottenham_data_r1.png
キャスター: **2025年**のヨーロッパリーグ王者が、いま最下位。デ・ゼルビ監督の**2年目**は、開幕5試合で勝ちなし。
  no_telop: true
  image: assets/stats/pl_tottenham_data_r2.png
キャスター: 1901年、まだリーグに入っていないのにFAカップを制した、唯一のクラブです。
  no_telop: true
  image: assets/stats/pl_tottenham_data_r2.png
キャスター: 創立は**1882年**。ホットスパーという名前で始まりました。
  no_telop: true
  image: assets/stats/pl_tottenham_data0.png
キャスター: 愛称は「スパーズ」。「リリーホワイツ」とも呼ばれます。
  no_telop: true
  image: assets/stats/pl_tottenham_data1.png
キャスター: ホームタウンは、ロンドン北部のトッテナムです。
  no_telop: true
  image: assets/stats/pl_tottenham_map.png
キャスター: 本拠地はトッテナム・ホットスパー・スタジアム。**6万2850人**が入ります。
  telop: 本拠地はトッテナム・ホットスパー・スタジアム。**6万2850人**が入ります
  image: assets/backgrounds/stadium_トッテナム.png
キャスター: オーナーは投資会社のイーニック。2001年からクラブを持っています。
  no_telop: true
  image: assets/stats/pl_tottenham_data4.png
キャスター: 1部リーグの優勝は**2回**。FAカップは8回、リーグカップは4回です。
  no_telop: true
  image: assets/stats/pl_tottenham_cups.png
キャスター: ヨーロッパリーグ3回、カップウィナーズカップ1回。合わせて4つ。
  no_telop: true
  image: assets/stats/pl_tottenham_cups.png
キャスター: プレミアでの最高位は、2016年から17年の**2位**です。
  no_telop: true
  image: assets/stats/pl_tottenham_last.png

## リーグに入らないまま、FAカップを獲った
@bg: assets/backgrounds/stadium_トッテナム_in.png

解説: 1888年に、フットボールリーグができました。
  telop: 1888年に、フットボールリーグができました
  source: 背景
  card: cup1901_card
  image: assets/images/pl_tottenham_story/scene.jpg
解説: それ以来、リーグの外のクラブがFAカップを獲った例は、ひとつだけ。
  telop: それ以来、リーグの外のクラブがFAカップを獲った例は、ひとつだけ
解説: それが1901年のトッテナム。**リーグに属していないまま**の優勝で、125年前のことです。
  telop: それが1901年のトッテナム。**リーグに属していないまま**の優勝で、125年前のことです
  image: assets/images/pl_tottenham_story/scene.jpg
解説: 1961年には、リーグとカップの両方を獲りました。**20世紀で最初の2冠**です。
  telop: 1961年には、リーグとカップの両方を獲りました。**20世紀で最初の2冠**です
  card: cup1901_3_card
解説: **イギリスのクラブで、ヨーロッパのタイトルを最初に獲ったのもトッテナム**でした。
  telop: **イギリスのクラブで、ヨーロッパのタイトルを最初に獲ったのもトッテナム**でした
  image: assets/images/pl_tottenham_story/scene.jpg
解説: 1963年のカップウィナーズカップ。国内カップの王者どうしで争った、いまは無い大会です。
  telop: 1963年のカップウィナーズカップ。国内カップの王者どうしで争った、いまは無い大会です
  image: assets/images/pl_tottenham_story/scene.jpg
解説: 1972年にはウエファカップも獲りました。
  telop: 1972年にはウエファカップも獲りました
  image: assets/images/pl_tottenham_story/scene.jpg
解説: 種類の違うヨーロッパのタイトルを2つ獲ったのも、イギリス勢では最初でした。
  telop: 種類の違うヨーロッパのタイトルを2つ獲ったのも、イギリス勢では最初でした
  card: cup1901_7_card
解説: ウエファカップは1984年にも獲りました。
  telop: ウエファカップは1984年にも獲りました
  image: assets/images/pl_tottenham_story/scene.jpg
解説: そして2025年のヨーロッパリーグで、ヨーロッパのタイトルは**4つ**になりました。
  telop: そして2025年のヨーロッパリーグで、ヨーロッパのタイトルは**4つ**になりました
  image: assets/images/pl_tottenham_story/scene.jpg

## このクラブの特徴
@bg: assets/backgrounds/stadium_トッテナム_in.png

解説: デ・ゼルビ監督のサッカーは、キーパーからつないで崩し、失えばすぐ前から奪い返す。
  telop: デ・ゼルビ監督のサッカーは、キーパーからつないで崩し、失えばすぐ前から奪い返す
  source: 背景
  card: features_card
  image: assets/photos/pl/tottenham_manager_57aac009.jpg
解説: 10億ポンドのスタジアムを持ち、稼ぎは世界9位。2025年秋、オーナーから1億ポンドの追加出資。
  telop: 10億ポンドのスタジアムを持ち、稼ぎは世界9位。2025年秋、オーナーから1億ポンドの追加出資
解説: 応援歌「グローリー・グローリー」は讃美歌から。1961年からの欧州の戦いで歌われ始めました。
  telop: 応援歌「グローリー・グローリー」は讃美歌から。1961年からの欧州の戦いで歌われ始めました
  image: assets/photos/pl/tottenham_manager_57aac009.jpg

## 雄鶏と拍車
@bg: assets/backgrounds/stadium_トッテナム_in.png

解説: クラブ名の由来は、中世の貴族ハリー・ホットスパー。エンブレムの雄鶏も、そこから来ています。
  telop: クラブ名の由来は、中世の貴族ハリー・ホットスパー。エンブレムの雄鶏も、そこから来ています
  source: 背景
  card: episode_card
  image: assets/images/pl_tottenham_episode/scene.jpg
解説: 戦いで馬に拍車を蹴り込むことから、ホットスパーと呼ばれた人物です。
  telop: 戦いで馬に拍車を蹴り込むことから、ホットスパーと呼ばれた人物です
解説: その拍車は、闘鶏の鶏にも付けるもの。だから雄鶏、というわけです。
  telop: その拍車は、闘鶏の鶏にも付けるもの。だから雄鶏、というわけです
  card: episode_2_card
解説: 1909年から10年のシーズンの終わり。元選手が**35ポンド**で、ブロンズの雄鶏を作りました。
  telop: 1909年から10年のシーズンの終わり。元選手が**35ポンド**で、ブロンズの雄鶏を作りました
  image: assets/images/pl_tottenham_episode/scene.jpg
解説: その雄鶏が、スタンドの屋根に置かれました。
  telop: その雄鶏が、スタンドの屋根に置かれました
  image: assets/images/pl_tottenham_episode/scene.jpg

## このクラブを語る3人
@bg: assets/backgrounds/stadium_トッテナム_in.png

キャスター: ハリー・ケインは**11歳**で入った生え抜き。4度の期限付き移籍を経て、定位置をつかみました。
  telop: ハリー・ケインは**11歳**で入った生え抜き。4度の期限付き移籍を経て、定位置をつかみました
  source: 背景
  card: legends_card
  image: assets/photos/pl/tottenham_legend_131bf2d1.jpg
キャスター: 2023年2月、シティ戦のゴールで、60年代のエース、グリーヴスのクラブ記録を抜きました。
  telop: 2023年2月、シティ戦のゴールで、60年代のエース、グリーヴスのクラブ記録を抜きました
キャスター: クラブ史上最多の**280得点**を残し、無冠のままバイエルンへ。プレミアの得点王は3回でした。
  telop: クラブ史上最多の**280得点**を残し、無冠のままバイエルンへ。プレミアの得点王は3回でした
  image: assets/photos/pl/tottenham_legend_131bf2d1.jpg

## このクラブを語る3人（2人目）
@bg: assets/backgrounds/stadium_トッテナム_in.png

キャスター: ソン・フンミンは、レバークーゼンから来た韓国代表。**2200万ポンド**はアジア人の最高額。
  telop: ソン・フンミンは、レバークーゼンから来た韓国代表。**2200万ポンド**はアジア人の最高額
  source: 背景
  card: legends2_card
  image: assets/photos/pl/tottenham_legend_41006aeb.jpg
キャスター: 2021年から22年、サラーと並ぶ**23得点**で得点王。アジアの選手では初めてでした。
  telop: 2021年から22年、サラーと並ぶ**23得点**で得点王。アジアの選手では初めてでした
キャスター: 10年目、主将としてヨーロッパリーグの決勝でユナイテッドに**1対0**。初めてのタイトルでした。
  telop: 10年目、主将としてヨーロッパリーグの決勝でユナイテッドに**1対0**。初めてのタイトルでした
  image: assets/photos/pl/tottenham_legend_41006aeb.jpg

## このクラブを語る3人（3人目）
@bg: assets/backgrounds/stadium_トッテナム_in.png

キャスター: レドリー・キングは生え抜きのセンターバック。クラブ一筋で、リーグ戦**268試合**を戦いました。
  telop: レドリー・キングは生え抜きのセンターバック。クラブ一筋で、リーグ戦**268試合**を戦いました
  source: 背景
  card: legends3_card
  image: assets/photos/pl/tottenham_legend_a1b6d580.jpg
キャスター: 2000年12月、ブラッドフォード戦で開始**10秒**のゴール。当時のプレミア最速記録でした。
  telop: 2000年12月、ブラッドフォード戦で開始**10秒**のゴール。当時のプレミア最速記録でした
キャスター: 膝の軟骨が無く、練習できないまま試合へ。2008年、主将としてリーグカップを掲げました。
  telop: 膝の軟骨が無く、練習できないまま試合へ。2008年、主将としてリーグカップを掲げました
  image: assets/photos/pl/tottenham_legend_a1b6d580.jpg

## 今季の監督
@bg: assets/backgrounds/stadium_トッテナム_in.png

キャスター: 監督はイタリア人のロベルト・デ・ゼルビ。**2026年3月**から、2年目です。
  telop: 監督はイタリア人のロベルト・デ・ゼルビ。**2026年3月**から、2年目です
  source: 報道
  card: manager_card
  image: assets/photos/pl/tottenham_manager_57aac009.jpg
キャスター: 前はマルセイユ。就任した昨季は17位で、最終節で残留を決めました。
  telop: 前はマルセイユ。就任した昨季は17位で、最終節で残留を決めました

## 今季の登録選手
@bg: assets/backgrounds/stadium_トッテナム_in.png

キャスター: 今季の登録選手は35人です。
  no_telop: true
  source: 報道
  image: assets/stats/pl_tottenham_gk.png
キャスター: ゴールキーパーは4人。
  no_telop: true
  image: assets/stats/pl_tottenham_gk_f.png
キャスター: マルティン・ドゥーブラフカはスロバキア代表で**60試合**。
  no_telop: true
  image: assets/stats/pl_tottenham_gk_f.png
キャスター: グリエルモ・ヴィカーリオはリーグ戦で**93試合**。
  no_telop: true
  image: assets/stats/pl_tottenham_gk_f.png
キャスター: ディフェンダーは12人。
  no_telop: true
  image: assets/stats/pl_tottenham_df_f.png
キャスター: 日本の高井幸大がいます。
  no_telop: true
  image: assets/stats/pl_tottenham_df_f.png
キャスター: 主将を務めるのが**ミッキー・ファン・デ・フェン**。
  no_telop: true
  image: assets/stats/pl_tottenham_df_f.png
キャスター: リーグ戦で**79試合**に出ています。
  no_telop: true
  image: assets/stats/pl_tottenham_df_f.png
キャスター: ベン・デイヴィスはウェールズ代表で**100試合**。
  no_telop: true
  image: assets/stats/pl_tottenham_df_f.png
キャスター: ミッドフィールダーは9人。
  no_telop: true
  image: assets/stats/pl_tottenham_mf_f.png
キャスター: ロドリゴ・ベンタンクールはウルグアイ代表で**77試合**。
  no_telop: true
  image: assets/stats/pl_tottenham_mf_f.png
キャスター: パペ・マタル・サールはセネガル代表で**42試合**。
  no_telop: true
  image: assets/stats/pl_tottenham_mf_f.png
キャスター: フォワードは10人。
  no_telop: true
  image: assets/stats/pl_tottenham_fw_f.png
キャスター: リシャルリソンはブラジル代表で**54試合**。
  no_telop: true
  image: assets/stats/pl_tottenham_fw_f.png
キャスター: デヤン・クルゼフスキはスウェーデン代表で**45試合**。
  no_telop: true
  image: assets/stats/pl_tottenham_fw_f.png

## 今季のここまで
@bg: assets/backgrounds/stadium_トッテナム_in.png

解説: プレミアリーグは5試合を終えて、**2引き分け3敗**です。
  telop: プレミアリーグは5試合を終えて、**2引き分け3敗**です
  source: 報道
  card: season_card
解説: 順位は**20位**、勝ち点は**2**です。
  telop: 順位は**20位**、勝ち点は**2**です
解説: いまは降格圏にいます。
  telop: いまは降格圏にいます
