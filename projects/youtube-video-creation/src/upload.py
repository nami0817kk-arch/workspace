...
def when_to_publish(clock: str, now=None) -> str:
    ...
    ``明日07:30`` … 翌日のその時刻（2026-09-09）。**夜に書き出して朝に出す**
                 ときに要る。`+750` のような分数での指定は、
                 書き出しに手間取ると狙いが狂う。

    **公開は7時から24時のあいだ**（2026-09-10 ユーザー決定）。
    深夜0〜4時に出したショートは中央値10回で、朝（1,035回）夕方（1,078回）と
    2桁ちがう。**枠の外はここで止める。**通してしまうと、
    書き出しの遅れが「気づいたら深夜」に化ける。
    ...
    （2026-09-10 に126本を実測。公開から2日以上たったものだけ。
    **本編は時間帯で差が無い。**効くのはショート）
    """
    from datetime import datetime, timedelta, timezone
