...
def test_short_gets_a_few_voices_at_the_end():
    ...
    # ~~語りだけの山場は、反応を1件だけ冒頭に上げる~~（2026-09-22）
    # → **2026-09-23 に取り消し**（ユーザー「最初にネットコメントがあってよく分からん」）。
    # 前提のない反応が題の直後に来ると、何の話か分からないまま始まる。
    # 反応は最後だけ。言葉を早く出すのは取材メモの側で直す（`_check_quote_timing` が止める）
    assert said[0] == "解説", "語りが先。反応を冒頭に繰り上げない"
    assert said[1] == "ネット民"
...
def test_一言のあとに題を読む回はショートでは題の行から始める():
    """2026-09-22: プレミア20クラブ紹介のショートが、クラブ名を一度も言わずに始まっていた。
    2026-09-23: 一言（4〜5秒）のせいで誰かの言葉が16秒に届かない回が20本中14本あったので、
    ショートは一言を落として題の行から始める（題にクラブ名が入る）。本編の一言はそのまま。"""
    from src.shorts import trim

    body = "\n".join([
        "---", "title: ボーンマスってどんなクラブ？", "---", "",
        "## オープニング", "", "キャスター: プレミアリーグで、いちばん小さなスタジアム。", "",
        "キャスター: ボーンマスってどんなクラブ？", "", "キャスター: 消えかけたクラブが、いまヨーロッパで戦っています。", "",
        "## 基礎DATA", "@main: true", "", "解説: 創立は1899年です。", "", "解説: 本拠地はディーン・コートです。", "",
    ])
    short = trim(parse_script(body))
