...
final List<GuideSection> guideSections = [
  ...
  const GuideSection(
    ...
    topics: [
      ...
      GuideTopic(
        title: 'スカウティングレポート(試合プレビュー)',
        description: '次の対戦相手の戦力・キープレイヤー・直近のフォームをまとめた事前情報。ここで判明した'
            'キープレイヤーにマンマークを付けるなど、試合前の戦術判断の材料になる。',
      ),
      GuideTopic(
        title: '逃げ切りモード',
  ...
  const GuideSection(
    ...
    topics: [
      ...
      GuideTopic(
        title: '天候',
        description: '試合当日の天候(晴れ/雨/強風/猛暑/雪)。悪天候ほど攻撃力とチャンスの数が下がり、'
            '雨は守備側がやや優位、猛暑は疲労が溜まりやすいなど展開が変わる。雨男・雪国育ちなど'
            '特定の天候で輝く選手特性を持つ選手は、天気予報に合わせた起用が有効になる。',
      ),
      GuideTopic(
        title: '記者会見',
        description: '試合後に記者の質問へ答えるイベント。回答の選択によってチームの士気が上下するため、'
            '試合結果に合ったコメントを選ぶこともマネジメントの一部になる。',
      ),
    ],
  ),
  const GuideSection(
    ...
    topics: [
      GuideTopic(
        ...
        description: 'チーム全体、または選手個別に設定する練習の重点分野。設定した分野に関連する能力値が'
            '伸びやすくなる。既定は「全体練習」で、ポジションに応じて攻守をバランス良く伸ばす。'
            '攻撃強化・守備強化・体力強化はその分野に特化する代わりに伸びが速く、'
            '「休養」は疲労を大きく回復する代わりに能力値が伸びない。',
      ),
      GuideTopic(
        title: '特訓ドリル',
        description: 'チーム/個別の方針とは別に、狙った能力値を集中的に伸ばす個別設定。1人あたり2つまで'
            '指定でき、同時に指定できる人数にはヘッドコーチのレベルに応じた上限がある。',
      ),
      GuideTopic(
        title: 'メンター制度',
      ...
      GuideTopic(
        title: 'ローテーション方針',
        description: '選手個別のトレーニング方針を複数登録しておくと、週替わりで自動的に切り替わる。'
            '攻撃と守備を交互に鍛えるなど、毎週手動で切り替えなくてもバランス良く育成できる。',
      ),
      GuideTopic(
        title: '個別声かけ・話し合い',
        description: '個別声かけ(モチベーショントーク)は選手の士気を高め、話し合いは不満度を和らげる'
            '監督コマンド。効果は選手の性格による感応度で変わり、どちらも同じ選手には数週間の'
            'クールダウンがある。',
      ),
      GuideTopic(
        title: '戦術ミーティング',
        description: 'チーム全体の戦術理解を深め、判断力・ポジショニング・チームワークといったメンタル系'
            '能力値をわずかに伸ばす。連発はできず、数週間のクールダウンがある。',
      ),
      GuideTopic(
        title: '自動トレーニング',
        description: '有効にすると、節を進めた際に週次トレーニングが自動で実施される。細かく管理したい'
            '週だけ無効に戻して手動で行うこともできる。',
      ),
    ],
  ),
  const GuideSection(
  ...
  const GuideSection(
    ...
    topics: [
      ...
      GuideTopic(
        title: '表彰・ベストイレブン・殿堂',
        description: 'シーズン終了時には得点王・MVP・ゴールデングラブ(無失点王)などの個人タイトルと'
            'シーズンベストイレブンが選出される。長く活躍した選手は引退・退団後に殿堂入りとして'
            '記録に残る。',
      ),
      GuideTopic(
